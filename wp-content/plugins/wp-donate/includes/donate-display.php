...
*/
    ...
    {		
		...
		$donation_type = mysql_real_escape_string($_REQUEST['donation_type']);
		$r_frequency = mysql_real_escape_string($_REQUEST['r_frequency']);
		$r_times = mysql_real_escape_string($_REQUEST['r_times']);
		$amount = mysql_real_escape_string($_REQUEST['amount']);
		$card_type = mysql_real_escape_string($_REQUEST['card_type']);
		$x_card_num = mysql_real_escape_string($_REQUEST['x_card_num']);
		$exp_month = mysql_real_escape_string($_REQUEST['exp_month']);
		$exp_year = mysql_real_escape_string($_REQUEST['exp_year']);
		$x_card_code = mysql_real_escape_string($_REQUEST['x_card_code']);
		...
		if($paypalp = authorizepayment($METHOD_TO_USE,$REQUEST,$address1,$city,$state,$country,$zipcode,$emailaddress,$plan))
		{
				...
				$wpdb->query(
						$wpdb->prepare("
							INSERT INTO `".$wpdb->prefix."donate`
							(`first_name`, `last_name`, `organization`, `address`, `city`, `country`, `state`, `zip`, `phone`, `email`, `donation_type`, `amount`, `comment`, `status`,`date`) 
